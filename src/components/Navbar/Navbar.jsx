...
const Navbar = (props) => {
    ...
    return (
        <div className="font-sans">
            <nav className="fixed z-50 w-full flex items-center h-[72px] p-2 px-4 bg-[#eff1f3] shadow-sm">
                <div className="mr-4">
                    <img src={logo} alt="OLX" className='w-11 cursor-pointer' onClick={() => navigate('/')} />
                </div>

                <div className='relative flex items-center bg-white border-[2px] border-[#002f34] rounded-[4px] h-12 w-[280px] mr-4 location-search'>
                    <img src={search} alt="Search" className='w-5 mx-2' />
                    <input placeholder='Search city, area, or locality...' className='w-full h-full outline-none text-[16px] bg-transparent' type="text" />
                    <img src={arrow} alt="Arrow" className='w-6 mx-2 cursor-pointer' />
                </div>

                <div className="main-search relative flex-grow flex items-center h-12">
                    <div className="flex w-full h-full border-[2px] border-[#002f34] rounded-[4px] bg-white overflow-hidden focus-within:border-[#23e5db]">
                        <input
                            ...
                            className='w-full h-full px-3 outline-none text-[16px] bg-transparent'
                            type="text"
                            value={searchQuery}
                ...
                <div className="lang items-center mx-4 cursor-pointer">
                ...
                {!user ? (
                    <button onClick={toggleModal} className='font-bold ml-4 mr-4 cursor-pointer text-[#002f34] text-[16px] hover:underline'>
                        Login
                    </button>
                ) : (
                    <div className="relative mx-4 flex items-center cursor-pointer" ref={menuRef}>
                        ...
                        {showMenu && (
                            <div className="absolute top-12 right-0 mt-2 w-48 bg-white shadow-lg rounded-[4px] border border-gray-200 z-50">
                                <ul className="text-[14px] text-[#002f34]">
                                    <li onClick={() => { navigate('/my-ads'); setShowMenu(false) }} className="p-3 px-4 hover:bg-[#eff1f3] cursor-pointer">My Ads</li>
                                    <li onClick={handleLogout} className="p-3 px-4 hover:bg-[#eff1f3] cursor-pointer text-red-500 font-bold border-t">Logout</li>
                                </ul>
                            </div>
                        )}
                    </div>
                ...
                <div onClick={toggleModalSell} className='cursor-pointer rounded-full p-[5px] bg-gradient-to-br from-[#ffce32] via-[#23e5db] to-[#3a77ff] shadow-md'>
                    <div className='flex items-center justify-center gap-1 bg-white text-[#002f34] font-bold text-[14px] uppercase h-10 px-5 rounded-full'>
                        <span className="text-xl mr-1">+</span> SELL
                    </div>
                </div>
            </nav>

            <div className='w-full z-40 bg-white shadow border-b pt-[72px]'>
                <div className="max-w-[1280px] mx-auto flex items-center h-12 px-4 text-[#002f34]">
                    <div className='flex items-center font-bold cursor-pointer mr-4'>
